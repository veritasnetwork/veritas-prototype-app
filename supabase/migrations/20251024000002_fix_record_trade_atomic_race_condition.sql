-- Fix Bug #1: Balance Calculation Race Condition
-- Move balance calculation inside record_trade_atomic with FOR UPDATE locks
-- This prevents TOCTOU race conditions when concurrent trades occur

-- Drop all existing versions of record_trade_atomic to avoid signature conflicts
...
CREATE OR REPLACE FUNCTION "public"."record_trade_atomic"(
  ...
  -- REMOVED: p_token_balance and p_belief_lock (now calculated internally)
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
COMMENT ON FUNCTION record_trade_atomic IS 'Atomically records trade, updates balances with FOR UPDATE locks to prevent race conditions, and applies stake skim. Bug #1 and #2 fixes included.';
