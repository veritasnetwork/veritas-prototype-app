-- Fix record_trade_atomic to not reference non-existent belief_lock_skim column
-- The skim calculation is still performed but we don't store it in trades table
...
BEGIN
  ...
  IF p_trade_type = 'buy' THEN
    -- Buy: add tokens, set lock to 2% of USDC spent
    v_token_balance := v_existing_balance + p_token_amount;
    v_belief_lock := FLOOR(p_usdc_amount * 0.02);
  ELSE
    -- Sell: subtract tokens, reduce lock proportionally
    v_token_balance := v_existing_balance - p_token_amount;

    IF v_existing_balance > 0 THEN
      v_belief_lock := FLOOR(v_existing_lock * (v_token_balance / v_existing_balance));
    ELSE
      v_belief_lock := 0;
    END IF;
  END IF;

  -- 3. Insert trade record (without belief_lock_skim column)
  INSERT INTO trades (
    pool_address,
    post_id,
    user_id,
