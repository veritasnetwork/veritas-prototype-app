-- Fix record_trade_atomic to not double-multiply USDC amounts
-- The p_usdc_amount parameter is already in micro-USDC units from the client
...
CREATE OR REPLACE FUNCTION record_trade_atomic(
  ...
  p_usdc_amount numeric,  -- Already in micro-USDC units
  p_tx_signature text,
  p_token_type text,
...
BEGIN
  -- p_usdc_amount is already in micro-USDC units, no conversion needed
  ...
  IF p_trade_type = 'buy' THEN
    v_belief_lock := (p_usdc_amount * 0.02)::bigint;  -- 2% of micro-USDC amount
  ELSE
  ...
  INSERT INTO trades (
    ...
    side,
    token_amount,
    usdc_amount,  -- Already in micro-USDC
    tx_signature,
    price_long,
    price_short,
    recorded_by,
    ...
    p_token_type,
    p_token_amount,
    p_usdc_amount,  -- Already in micro-USDC, no multiplication needed
    p_tx_signature,
    v_price_long,
    v_price_short,
    'server',
  ...
  -- 7. Upsert user_pool_balances with cumulative tracking
  INSERT INTO user_pool_balances (
    ...
    CASE WHEN p_trade_type = 'buy' THEN p_usdc_amount ELSE 0 END,
    CASE WHEN p_trade_type = 'sell' THEN p_usdc_amount ELSE 0 END,
    0,  -- Initial realized_pnl
    NOW(),
    CASE
      WHEN p_trade_type = 'buy' AND p_token_amount > 0
      THEN p_usdc_amount / p_token_amount / 1000000  -- Convert micro-USDC to USDC per token
      ELSE NULL
    END
  )
  ON CONFLICT (user_id, pool_address, token_type) DO UPDATE SET
    ...
    realized_pnl = user_pool_balances.realized_pnl +
      CASE
        ...
        THEN (p_usdc_amount / p_token_amount / 1000000 - user_pool_balances.entry_price) * p_token_amount
        ELSE 0
      END,
    last_trade_at = NOW(),
    entry_price = CASE
      ...
      THEN p_usdc_amount / p_token_amount / 1000000  -- New entry price
