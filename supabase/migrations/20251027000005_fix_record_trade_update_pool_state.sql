-- Fix record_trade_atomic to update pool_deployments with new state after each trade
-- This ensures the database reflects the current on-chain state (supplies, sqrt_prices, vault_balance)
...
CREATE OR REPLACE FUNCTION record_trade_atomic(
  ...
  -- ✅ NEW: Accept supplies from on-chain state after trade
...
BEGIN
  ...
  -- 9. ✅ NEW: Update pool_deployments with current state after trade
  -- This keeps the database in sync with on-chain state
  BEGIN
    -- Get current vault balance
    SELECT vault_balance
    INTO v_current_vault
    ...
    IF p_trade_type = 'buy' THEN
      -- On buy, vault increases by USDC amount
      v_current_vault := v_current_vault + v_usdc_micro;
    ELSE
      -- On sell, vault decreases by USDC amount
      v_current_vault := GREATEST(0, v_current_vault - v_usdc_micro);
    END IF;

    -- Update pool state with supplies, sqrt_prices, and vault_balance
    -- All values come from on-chain state after the trade
    IF p_s_long_after IS NOT NULL AND p_s_short_after IS NOT NULL THEN
      -- Full update with supplies
      UPDATE pool_deployments
      SET
        s_long_supply = p_s_long_after,
        s_short_supply = p_s_short_after,
        sqrt_price_long_x96 = p_sqrt_price_long_x96::numeric,
        sqrt_price_short_x96 = p_sqrt_price_short_x96::numeric,
        vault_balance = v_current_vault,
        updated_at = NOW()
      WHERE pool_address = p_pool_address;
    ELSE
      -- Partial update without supplies (backward compatibility)
      UPDATE pool_deployments
      SET
        sqrt_price_long_x96 = p_sqrt_price_long_x96::numeric,
        sqrt_price_short_x96 = p_sqrt_price_short_x96::numeric,
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    RAISE WARNING 'Error updating pool_deployments: %', SQLERRM;
  END;

  -- 10. Return success with trade details
