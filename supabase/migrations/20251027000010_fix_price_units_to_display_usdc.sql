-- Fix price_long and price_short to be stored in USDC (display units) per spec
-- Spec states: price_long/price_short should be "LONG/SHORT token price in USDC (display units)"
-- Currently stores in micro-USDC, causing unit inconsistency
...
BEGIN
  ...
  -- ✅ FIX: Convert to USDC (display units) by dividing by 1,000,000
