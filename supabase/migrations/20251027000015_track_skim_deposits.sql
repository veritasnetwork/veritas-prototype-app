-- Track skim deposits in custodian_deposits table for full audit trail
-- This creates an event record every time a buy trade deposits skim to custodian
...
BEGIN
  ...
  -- 10. ✅ Track skim deposit event in custodian_deposits (audit trail)
  -- This creates a permanent record of all stake deposits from trading
  IF p_trade_type = 'buy' AND v_skim_micro > 0 THEN
    INSERT INTO custodian_deposits (
      ...
      true,   -- Immediately credited since we're updating total_stake
      NOW()
    )
    ON CONFLICT (tx_signature) DO NOTHING;  -- Prevent duplicates if indexer already recorded

    -- Update agents.total_stake (custodian accounting)
    UPDATE agents
    SET
      total_stake = total_stake + v_skim_micro,
      total_deposited = total_deposited + p_skim_amount,  -- Track cumulative deposits
      updated_at = NOW()
    WHERE id = p_agent_id;
  END IF;
