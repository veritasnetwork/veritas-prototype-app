-- Fix skim tracking to be fully atomic and prevent double-crediting
-- The previous implementation had a bug where UPDATE agents would run even if INSERT failed
...
DECLARE
  ...
  v_deposit_inserted boolean;  -- Track if deposit was actually inserted
BEGIN
  ...
  -- 3. Calculate belief lock (2% of trade amount for buys, stored in micro-USDC)
  IF p_trade_type = 'buy' THEN
    v_belief_lock := v_usdc_micro * 0.02;
  ELSE
  ...
  -- 9. Update post total_volume_usdc
  UPDATE posts
  ...
  -- 10. ✅ ATOMIC: Track skim deposit and update stake ONLY if deposit is actually inserted
  -- This prevents double-crediting when event indexer has already recorded the deposit
  IF p_trade_type = 'buy' AND v_skim_micro > 0 THEN
    -- Try to insert deposit record
    INSERT INTO custodian_deposits (
      ...
      p_wallet_address,
      p_skim_amount,  -- Store in display USDC (per table spec)
      p_tx_signature,
      p_agent_id,
      'trade_skim',
      'server',
      false,  -- Will be confirmed by event indexer
      true,   -- Immediately credited
      NOW()
    )
    ON CONFLICT (tx_signature) DO NOTHING;

    -- Check if we actually inserted (vs conflict)
    -- Use GET DIAGNOSTICS to check if row was inserted
    GET DIAGNOSTICS v_deposit_inserted = ROW_COUNT;

    -- Only update total_stake if deposit was actually inserted
    -- This prevents double-crediting if event indexer already recorded this tx
  ...
  RETURN jsonb_build_object(
    ...
    'token_balance', v_token_balance,
    'belief_lock', v_belief_lock,
    'skim_credited', v_deposit_inserted  -- For debugging
  );
END;
$$;
