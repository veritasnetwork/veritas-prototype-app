-- Make deploy_pool_with_lock truly idempotent
-- Allow it to be called multiple times safely (by API route AND event indexer)
-- Uses INSERT ... ON CONFLICT to handle duplicate deployments gracefully
...
CREATE OR REPLACE FUNCTION deploy_pool_with_lock(
  ...
  p_s_scale_long_q64 numeric DEFAULT NULL,
  p_s_scale_short_q64 numeric DEFAULT NULL
)
RETURNS void
LANGUAGE plpgsql
AS $$
...
COMMENT ON FUNCTION deploy_pool_with_lock IS
'Records pool deployment with full idempotency. Can be called multiple times safely by both API route (immediate) and event indexer (async). Updates existing pool data if already exists.';
