-- Fix record_trade_atomic to use 'server' instead of 'api' for recorded_by
-- This fixes the constraint violation: trades_recorded_by_check
...
BEGIN
  ...
  WHERE user_id = p_user_id AND pool_address = p_pool_address
  FOR UPDATE;
  ...
  -- Insert trade record with 'server' as recorded_by (FIXED from 'api')
  INSERT INTO trades (
    pool_address,
    post_id,
    user_id,
    ...
    v_price_long,
    v_price_short,
    'server', -- FIXED: Changed from 'api' to 'server' to satisfy constraint
    true
  ...
  -- Update total_stake in beliefs table
