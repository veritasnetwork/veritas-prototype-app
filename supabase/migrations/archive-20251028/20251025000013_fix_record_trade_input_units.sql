-- Fix record_trade_atomic to properly handle input units
-- The p_usdc_amount parameter comes from the API in DISPLAY units (USDC with decimals)
-- It needs to be converted to micro-USDC for storage
...
CREATE OR REPLACE FUNCTION record_trade_atomic(
  ...
  p_usdc_amount numeric,  -- Input in DISPLAY units (USDC with decimals)
  p_tx_signature text,
  p_token_type text,
...
BEGIN
  -- Convert DISPLAY units to micro-USDC for storage
  ...
  INSERT INTO trades (
    ...
    side,
    token_amount,
    usdc_amount,  -- Store in micro-USDC
    tx_signature,
    price_long,
    price_short,
    recorded_by,
    ...
    p_token_type,
    p_token_amount,
    v_usdc_amount_micro,  -- Use converted micro-USDC value
    p_tx_signature,
    v_price_long,
    v_price_short,
    'server',
  ...
  INSERT INTO user_pool_balances (
    ...
    CASE
      ...
      THEN p_usdc_amount / p_token_amount  -- Already in display units, so price per token
      ELSE NULL
    END
  )
  ON CONFLICT (user_id, pool_address, token_type) DO UPDATE SET
    ...
    entry_price = CASE
      ...
      THEN p_usdc_amount / p_token_amount  -- Price per token in display units
