-- Fix record_trade_atomic to convert display USDC to micro-USDC
-- The p_usdc_amount parameter comes from the client in DISPLAY units (e.g., 1.5 for $1.50)
-- It needs to be converted to micro-USDC (e.g., 1500000) before storing in the database
...
CREATE OR REPLACE FUNCTION record_trade_atomic(
  ...
  p_usdc_amount numeric,  -- IN DISPLAY UNITS (USDC with decimals, e.g., 1.5 for $1.50)
  p_tx_signature text,
  p_token_type text,
