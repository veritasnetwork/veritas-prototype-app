-- Fix belief_lock calculation to use micro-USDC instead of display USDC
-- The belief_lock column is BIGINT (micro-USDC), but the function was calculating it
-- as display USDC (0.2) which got truncated to 0 when stored.

-- 1. Fix the record_trade_atomic function to calculate belief_lock in micro-USDC
CREATE OR REPLACE FUNCTION public.record_trade_atomic(
  ...
  p_usdc_amount numeric,  -- Received in display USDC
  p_tx_signature text,
  p_token_type text,
...
DECLARE
  ...
  v_belief_lock bigint;  -- Store in micro-USDC
...
BEGIN
  ...
  -- 3. Calculate belief lock (2% of trade amount, stored in micro-USDC)
  IF p_trade_type = 'buy' THEN
    v_belief_lock := (v_usdc_micro * 0.02)::bigint;  -- FIX: Use micro-USDC amount (was: p_usdc_amount * 0.02)
  ELSE
  ...
  INSERT INTO user_pool_balances (
    ...
    p_token_type,
    v_token_balance,
    v_belief_lock,  -- Now in micro-USDC
  ...
  ON CONFLICT (user_id, pool_address, token_type) DO UPDATE SET
    ...
    belief_lock = v_belief_lock,  -- Now in micro-USDC
  ...
  -- 8. Record implied relevance (if we have sqrt prices)
  BEGIN
    DECLARE
